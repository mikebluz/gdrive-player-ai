...
- **Bloom's first iteration is dropped on a cold-start play** (symptom: "no notes for several iterations, especially Bed"). Pressing Play resumes the AudioContext *asynchronously*; if the first `_ambTick` runs while the context is still `suspended`, its voices anchor to a `currentTime` that jumps forward on resume, so the first onset of every layer lands in the past and is dropped — worst on Bed (long iteration = seconds of silence). Fix in `_ambStartGenerator`: defer the first tick + `setInterval` until `Tone.getContext().rawContext.state === 'running'` (resolve via `Tone.start()`'s promise + a ~1.5 s poll fallback; guard re-entry with `E._starting` and bail if Stop cleared `cfg.playing`).
