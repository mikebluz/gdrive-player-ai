...
- **Still deferred (Unit-Sync/Bar-Lock only — NOT in the harness battery, so `check()` can't prove them; need ear-test):** `_ambLayerSubCount`, `_ambNaturalUnitSec`, per-generator default timing.
