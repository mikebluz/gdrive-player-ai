...
- **Deferred (needs a browser harness run to prove byte-identity):** the emit-dispatch router, `_ambLayerSubCount`, `_ambNaturalUnitSec`. These sit on the harness-covered note path and are intertwined with the scheduling mechanism (free `stepLayer` vs phase-anchored `windowLayer` vs extras chain, with differing emitter signatures) — convert only alongside a green harness.
