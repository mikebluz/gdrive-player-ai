    ...
    function _ambRenderHeader() {
      // Key readout (left of elapsed): workspace root + scale, kept live by
      // this same always-on rAF; click opens the key picker.
      try {
        const kEl = document.getElementById('bloom-hdr-key');
        if (kEl) {
          if (!kEl._wired) {
            kEl._wired = true;
