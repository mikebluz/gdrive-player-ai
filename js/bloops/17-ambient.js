    ...
    function _defaultAmbientConfig() {
      return {
        ...
        bed:     { on: true,  density: 4, register: 4, spread: 2, intervalMs: 4750, lengthMs: 6650, motion: 30, drift: 0, when: 'always', level: 70, panMode: 'spread', space: 0, strum: 0, strumFidelity: 0, tone: '', scale: '', attack: 2000, decay: 200, sustain: 85, release: 3650, fine: 0, mod: _ambDefaultMod(), ..._ambDefaultFx() },
        motif:   { on: false, register: 5, range: 2, proximity: 35, intervalMs: 1200, lengthMs: 1000, restProb: 30, twist: 0, accent: 0, drift: 0, when: 'always', level: 70, panMode: 'spread', space: 0, tone: '', scale: '', attack: 100, decay: 120, sustain: 70, release: 500, fine: 0, mod: _ambDefaultMod(), ..._ambDefaultFx() },
        texture: { on: false, register: 6, fill: 35, intervalMs: 450, lengthMs: 300, mutateRate: 40, drift: 0, when: 'always', level: 70, panMode: 'spread', space: 0, tone: '', scale: '', attack: 40, decay: 80, sustain: 0, release: 240, fine: 0, mod: _ambDefaultMod(), ..._ambDefaultFx() },
        beat:    { on: false, kit: 'tr808', gen: 'random', intervalMs: 500, lengthMs: 200, restProb: 25, bars: 1, pulses: 4, steps: 8, rotate: 0, rhythmVar: 0, drift: 0, when: 'always', level: 70, panMode: 'spread', space: 0, attack: 1, decay: 60, sustain: 70, release: 120, fine: 0, mod: _ambDefaultMod(), ..._ambDefaultFx() },
    ...
    function _normalizeSeqLayer(s, id) {
      ...
      ['intervalMs','lengthMs','drift','level','accent','varyDepth','returnN','returnChance','attack','decay','sustain','release','fine'].forEach(k => { if (!Number.isFinite(s[k])) s[k] = d[k]; });
    ...
    function _ambPublishWrap(name, step) {
      ...
      // Wrap-ensemble: capture each degree's TONE (parallel to intervals) when the step is a
    ...
    // Wrap-ensemble side-channel: _ambDegreeFreq stamps the picked degree's tone here so a
    ...
    // tones — so a layer falls back to its own Tone (the existing behaviour, unchanged).
    function _ambDegreeToneAt(deg, src) {
      if (!src || src.type !== 'wrap' || !Number.isFinite(src.id)) return null;
      const w = _ambFindWrap(src.id);
      if (!w || !Array.isArray(w.tones) || !w.tones.length) return null;
    ...
    // Per-layer Voice overrides applied to every note's params: ADSR (attack/
    // decay/sustain/release, ms / %) REPLACE whatever envelope a params builder
    ...
    // Voice-aware EUCLID emitter (Phase 4 Step 3): ONE onset engine, renderer
    // chosen by the layer's voice. Synth → a proximity-constrained scale-degree
    // walk (anchored to the root, resets per cycle); Kit → V interlocking drum
    // voices. Because the onset SELECTION (pattern + rhythmVar + restProb draws) is
    // identical either way, a Voice swap keeps the SAME onsets and only changes the
    // sound — no template switch. `phaseStore` is the layer's phase-anchor map
    // (bassPhase for a Bass-typed layer, runPhase for a Beat-typed one) so the
    // type-keyed When-cursor / boundary logic still finds its anchor.
    function _ambEmitEuclid(E, inst, key, now, horizon, lead, space, cfg, phaseStore, dfltType) {
    ...
    function _ambEmitArp(at, arp, space, key, play) {
      ...
      // position (time + entry/chord/step) so the popover can light the playing
      // step off the audible clock. Recorded regardless of mute/rest.
      if (_E && _E._arpLiveKey === key) { (st.live || (st.live = [])).push({ at: at, ei: _ei, ci: _ci, s: _stepIdx }); if (st.live.length > 96) st.live.splice(0, st.live.length - 96); }
    ...
    const BLOOPS_PERF_BUILD = 'press-start-1';
    ...
    const _AMB_PARAM_DESC = {
      ...
      fine: 'Fine pitch offset in cents (±100 = ±1 semitone), added to every note.',
    ...
    function _ambSeqLayerHtml(s, i) {
      ...
      return '<div class="ambient-layer collapsed" data-seq-id="' + id + '">' +
        ...
        // No Seed group — a Seq plays the pitches captured in its own sequence.
        // Groups follow the engine components (Voice · Generator · Timing ·
        // Variation · Mix) like the schema-driven cards.
        grp('Generator') +
    ...
    function _ambSampleLayerHtml(s, i) {
      ...
      return '<div class="ambient-layer collapsed" data-samp-id="' + id + '">' +
        ...
        // Groups follow the engine components: the chopper params are the
        // Generator; interval/length/drift/When are Timing.
        grp('Generator') +
    ...
    // Shared control fragments — every layer's schema is Voice + Source + a
    // generator-specific block + Timing + Mix. Factoring the invariant blocks out
    // keeps them single-source (a new shared control lands once, not in 9 schemas)
    ...
    const _AMB_LAYER_SCHEMA = {
      bed: { label: 'Bed', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Interval', 200, 12000, 50], ['unitsync'], ['sl', 'chordPhraseLen', 'Repeat', 1, 16, 'chords / phrase'], ['sl', 'chordRepeats', 'Times', 1, 16, 'phrase repeats'], ['tm', 'lengthMs', 'Length', 300, 16000, 100], ['sl', 'drift', 'Drift', 0, 99, 'phase offset'], ['choke'], ['cond'],
      ...
      motif: { label: 'Motif', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Interval', 100, 4000, 20], ['unitsync'], ['tm', 'lengthMs', 'Length', 80, 4000, 20], ['sl', 'drift', 'Drift', 0, 99, 'phase offset'], ['cond'],
      ...
      texture: { label: 'Texture', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Interval', 80, 2000, 10], ['unitsync'], ['tm', 'lengthMs', 'Length', 60, 2000, 10], ['sl', 'drift', 'Drift', 0, 99, 'phase offset'], ['cond'],
      ...
      beat: { label: 'Beat', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Interval', 80, 2000, 10], ['sl', 'bars', 'Phrase', 1, 8, 'bars (euclid)'], ['unitsync'], ['tm', 'lengthMs', 'Length', 60, 2000, 10], ['sl', 'drift', 'Drift', 0, 99, 'phase offset'], ['cond'],
      ...
      arp: { label: 'Arp', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Interval', 40, 2000, 10], ['sl', 'bars', 'Phrase', 1, 8, 'bars (euclid)'], ['unitsync'], ['tm', 'lengthMs', 'Length', 40, 2000, 10], ['sl', 'drift', 'Drift', 0, 99, 'phase offset'], ['cond'],
      ...
      bass: { label: 'Bass', ctrls: [
        ...
        ['grp', 'Timing'], ['sl', 'bars', 'Phrase', 1, 8, 'bars (seed length)'], ['unitsync'], ['tm', 'lengthMs', 'Length', 60, 2000, 20], ['cond'],
      ...
      run: { label: 'Riff', ctrls: [
        ...
        ['grp', 'Timing'], ['sl', 'bars', 'Bars', 1, 16, 'loop length'], ['unitsync'], ['tm', 'lengthMs', 'Length', 40, 2000, 10], ['cond'],
      ...
      drone: { label: 'Drone', ctrls: [
        ...
        ['grp', 'Timing'], ['rate'], ['tm', 'intervalMs', 'Unit', 200, 8000, 50], ['sl', 'hold', 'Hold', 1, 16, 'units held before re-strike'], ['unitsync'], ['cond'],
    ...
    function _ambWireInst(E, inst) {
      ...
      if (vsw) vsw.addEventListener('change', () => {
        ...
        if (vsw.value === 'kit') {
          ...
          // One consistent drum on the layer's own (mono) rhythm — faithful to the
          // pitched voice it came from, not a randomised full kit. (0 = kick.)
          if (L.drum == null) L.drum = 0;
          L.euclidVoices = 1;   // mono timing to match a pitched line
        }
        if (vsw.value === 'synth') {
        ...
        // old-voice hits (currently-sounding ones ring out) + reset lastAt so the
        // next tick re-emits the SAME onsets with the new voice.
        try {
          const t0 = (typeof Tone !== 'undefined' && Tone.now) ? Tone.now() : 0;
          if (typeof cancelBloomFutureVoices === 'function') cancelBloomFutureVoices(key, t0);
          const st = E[store] && E[store][key]; if (st) st.lastAt = t0;
        } catch (e) {}
        _ambRenderExtras(E);
        try { const wrap = _ambGet(E, 'ambient-extra-layers'); const card = wrap && wrap.querySelector('.ambient-layer[data-inst="' + key + '"]'); if (card) card.classList.remove('collapsed'); } catch (e) {}
        if (E.timer) { try { _ambSyncMods(); } catch (e) {} }
        if (typeof persistWorkspace === 'function') persistWorkspace();
      });
    ...
    function _ambShowArpChordEditor(E, getL) {
      ...
      const modal = document.createElement('div'); modal.className = 'step-div-modal amb-chordedit-modal';
      overlay.appendChild(modal);
      const close = () => { try { E._arpLiveKey = null; } catch (e) {} try { overlay.remove(); } catch (e) {} };
      overlay.addEventListener('click', (e) => { if (e.target === overlay) close(); });
      ...
      const renderSequence = (steps) => {
        ...
        steps.forEach((entry, ei) => {
          ...
          blocks.forEach((src, ci) => {
            ...
            if (chords) { const disp = dispOf(src); intervals = disp.iv.filter(iv => disp.m.indexOf(iv) < 0); rootPc = ((src.root | 0) % 12 + 12) % 12; }
            else { intervals = _ambScaleIntervals(src); rootPc = ((_ambSrcRootPc(src) | 0) % 12 + 12) % 12; }
            const N = Math.max(1, intervals.length);
            const len = N * octs;
            const passLen = Math.max(1, _ambArpPassLen(dir, len));
      ...
      const render = () => {
        ...
        h += '<div class="sm-footer"><button type="button" class="sm-apply amb-ce-ok">Done</button></div>';
        modal.innerHTML = h;
        // Track this arp's notes (for the live playhead) only while on Sequence.
        E._arpLiveKey = (tab === 'sequence') ? (L.type + ':' + L.id) : null;
        ...
        modal.querySelectorAll('.amb-seqcell').forEach(btn => btn.addEventListener('click', () => {
          const ei = btn.getAttribute('data-ei') | 0, ci = btn.getAttribute('data-ci') | 0, s = btn.getAttribute('data-s') | 0;
          const L2 = getL(); if (!L2 || !L2.steps || !L2.steps[ei]) return;
      ...
      // Live playhead: light the step that is AUDIBLY playing (recorded note times
